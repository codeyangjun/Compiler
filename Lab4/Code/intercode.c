...
void optArithmetic(){
	for(int i =0; i<IRlength;i++){
		...
		InterCode ir = IRList[i];
		if(ir->kind == PLUS_IR || ir->kind == MINUS_IR || ir->kind == STAR_IR || ir->kind == DIV_IR){
			if((ir->u.tripleOP.result->kind == TEMP_VAR_OP || ir->u.tripleOP.result->kind == VARIABLE_OP)
						&& ir->u.tripleOP.op1->kind == CONSTANT_OP && ir->u.tripleOP.op2->kind == CONSTANT_OP){
				
				int c1 = atoi(ir->u.tripleOP.op1->u.value);
				int c2 = atoi(ir->u.tripleOP.op2->u.value);
				int c3 = 0;
				switch(ir->kind){
					case PLUS_IR:
						c3 = c1+c2;
				...
				ir->kind = ASSIGN_IR;
				ir->u.doubleOP.right->kind = CONSTANT_OP;
				sprintf(ir->u.doubleOP.right->u.value, "%d", c3);
			}
		}
	}
}
